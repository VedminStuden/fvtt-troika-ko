...
export class TroikaActor extends Actor {
  ...
  static async create(data, options = {}) {
    ...
    if (data.type === "pc") {
      ...
      let content = ``;
      content += `<p><strong>기술:</strong> ${skill_roll.total} (${skill_roll.terms[0].results[0].result} + 3)</p>`;
      content += `<p><strong>체력:</strong> ${stamina_roll.total} (${stamina_roll.terms[0].results[0].result} + ${stamina_roll.terms[0].results[1].result} + 12)</p>`;
      content += `<p><strong>행운:</strong> ${luck_roll.total} (${luck_roll.terms[0].results[0].result} + 6)</p>`;

      ChatMessage.create({
        user: game.user._id,
        content: `<div class="incident-message"><h1>캐릭터 생성:<br>${data.name}</h1>${content}</div>`,
